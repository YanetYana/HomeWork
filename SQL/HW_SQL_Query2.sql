...
2) Используя JOIN, сделать выборку ачивок первых пяти игроков. Выбрать id игрока, имя игрока, email игрока и названия всех его достижений из таблиц PROFILES и ACHIEVEMENTS.

3) Используя Внутреннее соединение, выбрать всю информацию об игроках, которые одержали победу. Выбрать id, имена, фамилии, email, возраст и результат игры из таблиц PROFILES и GAME_RESULTS.

4) Используя команды JOIN выбрать логины и пароли всех игроков, которые были первыми (first_player_id) во всех играх. В результатирующую таблицу вывести поля login, password, id игры из таблиц USERS и GAMES

5) Используя команды JOIN, GROUP BY, COUNT(), ORDER BY посчитать количество игр которые провели игроки указанные в second_player_id. В результатирующую таблицу нужно вывести поля first_name, и количество игр. Полю с функцией COUNT() присвоить псевдоним count_of_games. Результат отсортировать от большего к меньшему
...
ADD CONSTRAINT FOREIGN KEY (profile_id) REFERENCES PROFILES (id);
...
first_name,
		 ...
		 achiev_name
FROM PROFILES AS pf
Inner JOIN PLAYER_ACHIEVS AS pa ON pf.id = pa.player_id
Inner JOIN ACHIEVEMENTS AS ac ON pa.achiev_id = ac.id
...
last_name,
		 ...
		 result_value
FROM PROFILES AS pf
Inner JOIN PLAYER_RESULTS AS pr ON pf.id = pr.player_id
Inner JOIN GAMES_RESULT AS gr ON pr.result_id = gr.id
WHERE gr.id =1 
...
first_player_id
FROM PROFILES AS pf
Inner JOIN GAMES AS gm ON gm.first_player_id = pf.id
Inner JOIN USERS AS us ON us.profile_id = pf.id

SELECT
first_name,
COUNT(second_player_id) AS count_of_games 
FROM PROFILES AS pf
Inner JOIN GAMES AS gm ON gm.second_player_id = pf.id
GROUP BY first_name
ORDER BY count_of_games DESC 

SELECT first_name, email
FROM PROFILES 
WHERE id IN (SELECT first_player_id FROM GAMES)
